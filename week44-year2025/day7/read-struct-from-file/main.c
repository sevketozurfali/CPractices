...
int main(void) {
    ...
    FILE *write_student = fopen("student_info.txt", "wb");
    fwrite(&latest_student, sizeof(student), 1, write_student);

    fclose(write_student);

    FILE* read_student = fopen("student_info.txt", "rb");

    student student_form_file;
    fread(&student_form_file, sizeof(student), 1, read_student);

    print_student(&student_form_file);
